import type { AgentPresetId, Config, WorkspacePaths } from './types.js';

export const AGENT_PROMPT_PLACEHOLDER = '$GH_AGENT_PROMPT';
export const LEGACY_AGENT_PROMPT_PLACEHOLDER = '$prompt';

export type BuiltInAgentPresetId = Exclude<AgentPresetId, 'custom'>;
...
export const CUSTOM_AGENT_PRESET_ID = 'custom' as const;
...
function doesCommandMatchPreset(
  ...
  return preset.commandTemplate === command;
}

export function inferAgentPresetIdFromCommand(command: string): AgentPresetId {
  for (const preset of AGENT_PRESETS) {
    if (
      preset.id !== CUSTOM_AGENT_PRESET_ID &&
      doesCommandMatchPreset(command, preset.id)
    ) {
      return preset.id;
    }
  }

  return CUSTOM_AGENT_PRESET_ID;
}
...
export function resolveAgentRuntimeEnvironment(input: {
  ...
  return {
    [preset.configEnv]: input.paths.root,
  };
}
...
export function resolveAgentPresetCommand(input: {
  presetId: AgentPresetId;
  customCommand?: string | null;
}): string {
  if (input.presetId === CUSTOM_AGENT_PRESET_ID) {
  ...
  return resolveAgentPresetCommandTemplate(input.presetId);
}
