...
export function buildRichSessionPrompt(input: {
  ...
  return [
    ...
    '',
    '[npx gh-agent 핵심 명령 가이드]',
    '- mailbox: npx gh-agent mailbox list | show <threadId> | promote <threadId...> | ignore <threadId...>',
    '- task: npx gh-agent task list | show <taskId> | create ... | update <taskId> ... | ready/doing/wait/done <taskId...>',
    '- 상태 확인: npx gh-agent status',
    '',
    '[GitHub 소통 원칙]',
