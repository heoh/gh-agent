...
export function buildRichSessionPrompt(input: {
  ...
  return [
    ...
    '',
    '[Session Command Guide]',
    '- Mailbox triage: use `gh-agent mailbox list`, `show <threadId>`, and `promote`/`ready`/`wait`/`ignore` to classify notifications and convert them into tracked work when needed.',
    '- Actionable tasks: use `gh-agent task list`, `show <taskId>`, `create`, `update`, and `ready`/`doing`/`wait`/`done` to advance project cards.',
    '- Quick diagnostics: use `gh-agent status` for lock, auth, and signal checks before or during a session.',
    '- Keep the execution loop explicit: triage mailbox, act on ready/doing work, sync GitHub records, then re-check the mailbox.',
    '',
