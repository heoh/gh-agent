...
- 런타임은 `GH_AGENT_PROMPT` env를 주입한다.
- 런타임은 `GH_AGENT_HOME=<workspace root>` env도 함께 주입한다.
...
생성되는 세션 프롬프트는 실행 루프를 다시 명시해야 한다.

- mailbox triage -> actionable work execution -> GitHub sync -> mailbox re-check
- 신뢰 가능한 지침 영역에서 `gh-agent mailbox ...`, `gh-agent task ...`, `gh-agent status` 사용 맥락을 다시 알려준다.
