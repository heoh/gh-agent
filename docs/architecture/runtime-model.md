...
- preset이 isolated config env를 지원하면 런타임은 `GH_AGENT_HOME/.gh-agent/agent-config/<preset>` 경로를 해당 CLI env (`CODEX_HOME`, `COPILOT_HOME`, `GEMINI_CLI_HOME`, `CURSOR_CONFIG_DIR`, `CLINE_DIR`) 로 주입한다.
