...
4. wake 규칙을 평가한다.
5. should_wake=true이면 agent session을 실행한다.
6. 세션 종료 후 운영 상태를 저장한다.
7. 다시 sleeping 폴링 루프로 복귀한다.
...
1. 시스템이 세션 ID를 발급한다.
2. 시스템이 브리핑 문맥을 준비한다.
3. 시스템이 에이전트 CLI를 실행한다.
4. 에이전트가 triage, board update, GitHub actions를 수행한다.
5. 에이전트가 종료한다.
6. 시스템이 종료 상태를 기록한다.
7. 시스템이 sleeping 폴링으로 돌아간다.
